...
class EndWayHandler():
    ...
    def createArea(self):
        ...
        borders.append(p2)
        # else:
        #         self.clustConnectors[cluster.id] = len(borders)-1
        # else:   # clustType == 'end'
        ...
        # plotPolygon(self.area,'b',1,False,True,True)
        # for id, connector in self.wayConnectors.items():
        #     side = 'S' if id>0 else 'E'
        #     p = self.area[connector]
        #     plt.text(p[0],p[1],'   '+str(abs(id))+' '+side,fontsize=12)
        # for id, connector in self.clustConnectors.items():
        #     side = 'S' if id>0 else 'E'
        #     p = self.area[connector]
        #     plt.text(p[0],p[1],'   C'+str(abs(id))+' '+side,fontsize=12,color='r')
        # plotCluster(cluster,False,True)
        # plotEnd()
