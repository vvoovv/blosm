...
def setup(app, osm):
    ...
    osm.addCondition(
        lambda tags, e: tags.get("power") == "line",
        "power_lines",
        wayManager
    )
    ...
    osm.addNodeCondition(
        lambda tags, e: tags.get("power") == "tower", # a condition
        "power_towers", # layer name
        None,
        # the second argument is the path to a Blender file with 3D models
        # the third argument is the name of the Blender file with 3D models
        # the fourth argument is the name of the collection that contains the related 3D model(s)
        BaseNodeRenderer(app, "D://path//to", "power_tower.blend", "power_tower")
    )
    # custom setup ends
