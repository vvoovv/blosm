...
from lib.blosm_networkx.utils import arbitrary_element
from lib.blosm_networkx.utils.decorators import not_implemented_for
...
def line_graph(G, create_using=None):
    ...
    ----------
    G : graph
        A blosm_networkx Graph, DiGraph, MultiGraph, or MultiDigraph.
...
def _lg_directed(G, create_using=None):
    ...
    create_using : blosm_networkx graph constructor, optional
       Graph type to create. If graph instance, then cleared before populated.
...
def inverse_line_graph(G):
    ...
    if nx.number_of_selfloops(G) != 0:
        msg = (
            "A line graph as generated by blosm_networkx has no selfloops, so G has no "
