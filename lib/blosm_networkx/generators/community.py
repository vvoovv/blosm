...
def random_partition_graph(sizes, p_in, p_out, seed=None, directed=False):
    ...
    Returns
    -------
    G : blosm_networkx Graph or DiGraph
...
def stochastic_block_model(
    ...
    if len(sizes) != len(p):
        raise nx.py_networkxException("'sizes' and 'p' do not match.")
    # Check for probability symmetry (undirected) and shape (directed)
    for row in p:
        if len(p) != len(row):
            raise nx.py_networkxException("'p' must be a square matrix.")
    if not directed:
        p_transpose = [list(i) for i in zip(*p)]
        for i in zip(p, p_transpose):
            for j in zip(i[0], i[1]):
                if abs(j[0] - j[1]) > 1e-08:
                    raise nx.py_networkxException("'p' must be symmetric.")
    # Check for probability range
    for row in p:
        for prob in row:
            if prob < 0 or prob > 1:
                raise nx.py_networkxException("Entries of 'p' not in [0,1].")
    # Check for nodelist consistency
    if nodelist is not None:
        if len(nodelist) != sum(sizes):
            raise nx.py_networkxException("'nodelist' and 'sizes' do not match.")
        if len(nodelist) != len(set(nodelist)):
            raise nx.py_networkxException("nodelist contains duplicate.")
    else:
...
def _powerlaw_sequence(gamma, low, high, condition, length, max_iters, seed):
    ...
    value, :exc:`~blosm_networkx.exception.ExceededMaxIterations` is raised.
...
def _generate_communities(degree_seq, community_sizes, mu, max_iters, seed):
    ...
    :exc:`~blosm_networkx.exception.ExceededMaxIterations` is raised.
...
def LFR_benchmark_graph(
    ...
    Basic usage::
        ...
        >>> from lib.blosm_networkx.generators.community import LFR_benchmark_graph
        >>> n = 250
        >>> tau1 = 3
        >>> tau2 = 1.5
        >>> mu = 0.1
