...
from lib.blosm_networkx.utils import py_random_state, random_weighted_sample
...
def _configuration_model(
    ...
    ``create_using`` see :func:`~blosm_networkx.empty_graph`.
...
def configuration_model(deg_sequence, create_using=None, seed=None):
    ...
    create_using : blosm_networkx graph constructor, optional (default MultiGraph)
    ...
    :mod:`~blosm_networkx.utils.random_sequence` (or one of your own). For
    ...
    if G.is_directed():
        raise nx.py_networkxNotImplemented("not implemented for directed graphs")
...
def directed_configuration_model(
    ...
    create_using : blosm_networkx graph constructor, optional (default MultiDiGraph)
        Graph type to create. If graph instance, then cleared before populated.
    seed : integer, random_state, or None (default)
        Indicator of random number generation state.
        See :ref:`Randomness<randomness>`.
...
def expected_degree_graph(w, seed=None, selfloops=True):
    ...
    blosm_networkx uses the standard convention that a self-loop edge counts 2
...
def havel_hakimi_graph(deg_sequence, create_using=None):
    ...
    create_using : blosm_networkx graph constructor, optional (default=nx.Graph)
        Graph type to create. If graph instance, then cleared before populated.
        Directed graphs are not allowed.

    Raises
    ------
    py_networkxException
...
def directed_havel_hakimi_graph(in_deg_sequence, out_deg_sequence, create_using=None):
    ...
    out_deg_sequence : list of integers
        Each list entry corresponds to the out-degree of a node.
    create_using : blosm_networkx graph constructor, optional (default DiGraph)
        Graph type to create. If graph instance, then cleared before populated.
...
class DegreeSequenceRandomGraph:
    ...
    def __init__(self, degree, rng):
        if not nx.is_graphical(degree):
            raise nx.py_networkxUnfeasible("degree sequence is not graphical")
        self.rng = rng
    ...
    def phase3(self):
        ...
        while self.remaining_degree:
            if not self.suitable_edge():
                raise nx.py_networkxUnfeasible("no suitable edges left")
            while True:
