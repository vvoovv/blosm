...
of `blosm_networkx.backends` with a key pointing to the handler.

For example::

    entry_points={'blosm_networkx.backends': 'sparse = py_networkx_backend_sparse'}

The backend must create a Graph-like object which contains an attribute
``__py_networkx_backend__`` with a value of the entry point name.

Continuing the example above::

    class WrappedSparse:
        __py_networkx_backend__ = "sparse"
        ...

When a dispatchable blosm_networkx algorithm encounters a Graph-like object
with a ``__py_networkx_backend__`` attribute, it will look for the associated
...
environment variable ``py_networkx_TEST_BACKEND`` is set to a registered
backend key, the dispatch machinery will automatically convert regular
blosm_networkx Graphs and DiGraphs to the backend equivalent by calling
``<backend dispatcher>.convert_from_nx(G, edge_attrs=edge_attrs, name=name)``.
Set ``py_networkx_FALLBACK_TO_NX`` environment variable to have tests
use blosm_networkx graphs for algorithms not implemented by the backend.

The arguments to ``convert_from_nx`` are:

- ``G`` : blosm_networkx Graph
...
to a form expected by the blosm_networkx tests.

By defining ``convert_from_nx`` and ``convert_to_nx`` methods and setting
the environment variable, blosm_networkx will automatically route tests on
dispatchable algorithms to the backend, allowing the full blosm_networkx test
suite to be run against the backend implementation.

Example pytest invocation::

    py_networkx_TEST_BACKEND=sparse pytest --pyargs blosm_networkx
...
It will be called with the list of blosm_networkx tests discovered. Each item
...
from lib.blosm_networkx.exception import NetworkXNotImplemented
...
def _get_backends(group, *, load_and_call=False):
    ...
    for ep in items:
        if ep.name in rv:
            warnings.warn(
                f"blosm_networkx backend defined more than once: {ep.name}",
                RuntimeWarning,
                stacklevel=2,
            )
        elif load_and_call:
...
backends = _get_backends("blosm_networkx.plugins")
backend_info = _get_backends("blosm_networkx.plugin_info", load_and_call=True)

backends.update(_get_backends("blosm_networkx.backends"))
backend_info.update(_get_backends("blosm_networkx.backend_info", load_and_call=True))
...
class _dispatch:
    ...
    graphs : str or dict or None, default "G"
        ...
        Use ``graphs=None`` if *no* arguments are blosm_networkx graphs such as for
    ...
    # These class attributes are currently used to allow backends to run blosm_networkx tests.
    # For example: `PYTHONPATH=. pytest --backend graphblas --fallback-to-nx`
    # Future work: add configuration to control these
    _is_testing = False
    _fallback_to_nx = (
        os.environ.get("py_networkx_FALLBACK_TO_NX", "true").strip().lower() == "true"
    )
    _automatic_backends = [
        x.strip()
        for x in os.environ.get("py_networkx_AUTOMATIC_BACKENDS", "").split(",")
        if x.strip()
    ]
    ...
    def __call__(self, /, *args, backend=None, **kwargs):
        ...
        if self._is_testing and self._automatic_backends and backend_name is None:
            # Special path if we are running blosm_networkx tests with a backend.
        ...
        if self.list_graphs:
            ...
            has_backends = any(
                hasattr(g, "__py_networkx_backend__")
                if gname not in self.list_graphs
                else any(hasattr(g2, "__py_networkx_backend__") for g2 in g)
                for gname, g in graphs_resolved.items()
            )
            if has_backends:
                graph_backend_names = {
                    getattr(g, "__py_networkx_backend__", "blosm_networkx")
                    for gname, g in graphs_resolved.items()
                    if gname not in self.list_graphs
                }
                for gname in self.list_graphs & graphs_resolved.keys():
            ...
            has_backends = any(
                hasattr(g, "__py_networkx_backend__") for g in graphs_resolved.values()
            )
            if has_backends:
                graph_backend_names = {
                    getattr(g, "__py_networkx_backend__", "blosm_networkx")
        ...
        if has_backends:
            ...
            # but we may be able to convert additional blosm_networkx graphs.
            backend_names = graph_backend_names - {"blosm_networkx"}
            ...
            if (
                "blosm_networkx" in graph_backend_names
                and graph_backend_name not in self._automatic_backends
            ):
                # Not configured to convert blosm_networkx graphs to this backend
                raise TypeError(
                    f"Unable to convert inputs and run {self.name}. "
                    f"{self.name}() has blosm_networkx and {graph_backend_name} graphs, but blosm_networkx is not "
                    f"configured to automatically convert graphs from lib.blosm_networkx to {graph_backend_name}."
                )
            backend = _load_backend(graph_backend_name)
            if hasattr(backend, self.name):
                if "blosm_networkx" in graph_backend_names:
                    # We need to convert blosm_networkx graphs to backend graphs
            ...
            raise py_networkxNotImplemented(
        ...
        # Only blosm_networkx graphs; try to convert and run with a backend with automatic
        ...
        # Default: run with blosm_networkx on blosm_networkx inputs
        return self.orig_func(*args, **kwargs)
    ...
    def _convert_arguments(self, backend_name, args, kwargs):
        ...
        # It should be safe to assume that we either have blosm_networkx graphs or backend graphs.
        ...
        for gname in self.graphs:
            if gname in self.list_graphs:
                bound.arguments[gname] = [
                    ...
                    if getattr(g, "__py_networkx_backend__", "blosm_networkx") == "blosm_networkx"
                    else g
                ...
                if getattr(graph, "__py_networkx_backend__", "blosm_networkx") == "blosm_networkx":
    ...
    def _convert_and_call(self, backend_name, args, kwargs, *, fallback_to_nx=False):
        ...
        except (NotImplementedError, py_networkxNotImplemented) as exc:
            if fallback_to_nx:
                return self.orig_func(*args, **kwargs)
            raise
    ...
    def _convert_and_call_for_tests(
        ...
        if isinstance(converted_result, nx.Graph) and self.name not in {
            ...
            # the same between the backend and blosm_networkx, then return the original
            # blosm_networkx result so the iteration order will be consistent in tests.
...
if os.environ.get("_py_networkx_BUILDING_DOCS_"):
