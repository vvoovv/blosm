"""
Miscellaneous Helpers for blosm_networkx.

These are not imported into the base blosm_networkx namespace but
can be accessed, for example, as

>>> import lib.blosm_networkx
>>> blosm_networkx.utils.make_list_of_ints({1, 2, 3})
[1, 2, 3]
>>> blosm_networkx.utils.arbitrary_element({5, 1, 7})  # doctest: +SKIP
1
"""
...
# see G.add_nodes and others in Graph Class in blosm_networkx/base.py
    ...
    if not isinstance(sequence, list):
        ...
        for i in sequence:
            ...
            try:
                ...
                raise nx.py_networkxError(errmsg) from None
            if ii != i:
                raise nx.py_networkxError(errmsg)
            result.append(ii)
        return result
...
def groups(many_to_one):
    ...
    Examples
    --------
    >>> from lib.blosm_networkx.utils import groups
...
class PythonRandomInterface:
    ...
    #    Commented out versions are not used by blosm_networkx
