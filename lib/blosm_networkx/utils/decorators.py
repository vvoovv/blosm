...
from lib.blosm_networkx.utils import create_py_random_state, create_random_state
...
def not_implemented_for(*graph_types):
    ...
    def _not_implemented_for(g):
        if (mval is None or mval == g.is_multigraph()) and (
            ...
            raise nx.py_networkxNotImplemented(errmsg)
...
def nodes_or_number(which_args):
    ...
    def _nodes_or_number(n):
        try:
            ...
            if n < 0:
                raise nx.py_networkxError(f"Negative number of nodes not valid: {n}")
        return (n, nodes)
...
class argmap:
    ...
    *args : iterable of (int, str or tuple)
    ...
    In the blosm_networkx codebase however, `argmap` is used within a function to
    ...
    **Try Finally**
    ...
    every import of blosm_networkx, at the cost of compiling upon the first call
    ...
    random_state
    py_random_state
    blosm_networkx.community.quality.require_partition
    require_partition
    ...
    def _lazy_compile(func):
        ...
        Notes
        -----
        It was observed in blosm_networkx issue #4732 [1] that the import time of
        blosm_networkx was significantly bloated by the use of decorators: over half
        ...
        [1] https://github.com/blosm_networkx/blosm_networkx/issues/4732
    ...
    def __call__(self, f):
        ...
        if self._finally and func.__is_generator:
            raise nx.py_networkxError("argmap cannot decorate generators with try_finally")
    ...
    def assemble(self, f):
        ...
        def get_name(arg, first=True):
            ...
            if arg in applied:
                raise nx.py_networkxError(f"argument {arg} is specified multiple times")
            applied.add(arg)
            if arg in sig.names:
                return sig.names[arg]
            elif isinstance(arg, str):
                if sig.kwargs is None:
                    raise nx.py_networkxError(
    ...
    * will issue a warning when passed as a positional argument.
    ...
    def _inner_deprecate_positional_args(f):
        ...
        def inner_f(*args, **kwargs):
            ...
            warnings.warn(
                (
                    f"Pass {args_msg} as keyword args. from lib.blosm_networkx version "
