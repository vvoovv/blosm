...
def generate_network_text(
    ...
    This notation is original to blosm_networkx, although it is simple enough that it
...
def forest_str(graph, with_labels=True, sources=None, write=None, ascii_only=False):
    ...
    if len(graph.nodes) > 0:
        if not nx.is_forest(graph):
            raise nx.py_networkxNotImplemented("input must be a forest or the empty graph")
