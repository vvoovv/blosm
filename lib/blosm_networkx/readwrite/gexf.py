...
from lib.blosm_networkx.utils import open_file
...
def read_gexf(path, node_type=None, relabel=False, version="1.2draft"):
    ...
    relabel : bool (default: False)
       ...
       instead of the node "id" attribute as the blosm_networkx node label.
    version : string (default: 1.2draft)
    Version of GEFX File Format (see http://gexf.net/schema.html)
       Supported values: "1.1draft", "1.2draft"
...
class GEXFWriter(GEXF):
    ...
    def __init__(
        ...
        subelement_text = f"blosm_networkx {nx.__version__}"
...
class GEXFReader(GEXF):
    ...
    def add_edge(self, G, edge_element, edge_attr):
        ...
        # blosm_networkx uses them as keys in multigraphs
        # if py_networkx_key is not specified as an attribute
        edge_id = edge_element.get("id")
        if edge_id is not None:
            data["id"] = edge_id

        # check if there is a 'multigraph_key' and use that as edge_id
        multigraph_key = data.pop("py_networkx_key", None)
...
def relabel_gexf_graph(G):
    ...
    ----------
    G : graph
       A blosm_networkx graph read from GEXF data

    Returns
    -------
    H : graph
      A blosm_networkx graph with relabeled nodes
    ...
    Notes
    -----
    This function relabels the nodes in a blosm_networkx graph with the
