...
from lib.blosm_networkx.utils import py_random_state
...
def branching_weight(G, attr="weight", default=1):
    ...
    You must access this function through the blosm_networkx.algorithms.tree module.
...
def greedy_branching(G, attr="weight", default=1, kind="max", seed=None):
    ...
    if kind not in KINDS:
        raise nx.py_networkxException("Unknown value for `kind`.")
...
class MultiDiGraph_EdgeKey(nx.MultiDiGraph):
    ...
    def __init__(self, incoming_graph_data=None, **attr):
        ...
        msg = "MultiDiGraph_EdgeKey has been deprecated and will be removed in blosm_networkx 3.4."
...
class Edmonds:
    ...
    def __init__(self, G, seed=None):
        ...
        msg = "Edmonds has been deprecated and will be removed in blosm_networkx 3.4. Please use the appropriate minimum or maximum branching or arborescence function directly."
        warnings.warn(msg, DeprecationWarning)
    ...
    def _init(self, attr, default, kind, style, preserve_attrs, seed, partition):
        ...
        - Check that the kind argument is in {min, max} or raise a py_networkxException.
...
def maximum_spanning_arborescence(
    ...
    if not is_arborescence(B):
        raise nx.exception.py_networkxException("No maximum spanning arborescence in G.")
...
def minimum_spanning_arborescence(
    ...
    if not is_arborescence(B):
        raise nx.exception.py_networkxException("No minimum spanning arborescence in G.")
    ...
    def _partition(self, partition, partition_arborescence):
        ...
        for e in partition_arborescence.edges:
            ...
            if e not in partition.partition_dict:
                ...
                except nx.py_networkxException:
                    pass
