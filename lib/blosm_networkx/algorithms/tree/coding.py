...
def from_nested_tuple(sequence, sensible_relabeling=False):
    ...
    def _make_tree(sequence):
        ...
        This function employs the :func:`~blosm_networkx.tree.join` function
...
def to_prufer_sequence(T):
    ...
    *n* - 1, inclusive. The tree corresponding to a given Prüfer
    ...
    the latter form. You can use :func:`~blosm_networkx.relabel_nodes` to
    ...
    if n < 2:
        ...
        raise nx.py_networkxPointlessConcept(msg)
    if not nx.is_tree(T):
        raise nx.NotATree("provided graph is not a tree")
