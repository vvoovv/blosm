...
def join(rooted_trees, label_attribute=None):
    ...
    .. deprecated:: 3.2
       ...
       `join` is deprecated in blosm_networkx v3.2 and will be removed in v3.4.
...
def join_trees(rooted_trees, *, label_attribute=None, first_label=0):
    ...
    ----------
    rooted_trees : list
        A list of pairs in which each left element is a blosm_networkx graph
    ...
    Notes
    -----
    Trees are stored in blosm_networkx as blosm_networkx Graphs. There is no specific
    enforcement of the fact that these are trees. Testing for each tree
    can be done using :func:`blosm_networkx.is_tree`.
