...
from lib.blosm_networkx.algorithms.components import connected_components
from lib.blosm_networkx.utils import arbitrary_element, not_implemented_for
...
def find_induced_nodes(G, s, t, treewidth_bound=sys.maxsize):
    ...
    ----------
    G : graph
      A chordal blosm_networkx graph
    s : node
    ...
    py_networkxError
        ...
        :exc:`py_networkxError` is raised.
        The algorithm can only be applied to chordal graphs. If the input
        graph is found to be non-chordal, a :exc:`py_networkxError` is raised.
    ...
    """
    if not is_chordal(G):
        raise nx.py_networkxError("Input graph is not chordal.")
...
def _is_complete_graph(G):
    ...
    if nx.number_of_selfloops(G) > 0:
        raise nx.py_networkxError("Self loop found in _is_complete_graph()")
    n = G.number_of_nodes()
    if n < 2:
        return True
    e = G.number_of_edges()
    ...
    while unnumbered:  # and current_treewidth <= treewidth_bound:
        ...
        if _is_complete_graph(sg):
            ...
            if current_treewidth > treewidth_bound:
                raise nx.py_networkxTreewidthBoundExceeded(
...
def complete_to_chordal_graph(G):
    ...
    Examples
    --------
    >>> from lib.blosm_networkx.algorithms.chordal import complete_to_chordal_graph
    >>> G = nx.wheel_graph(10)
