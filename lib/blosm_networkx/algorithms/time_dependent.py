...
def cd_index(G, node, time_delta, *, time="time", weight=None):
    ...
    ----------
    G : graph
       A directed blosm_networkx graph whose nodes have `time` attributes and optionally
    ...
    if not all(time in G.nodes[n] for n in G):
        raise nx.py_networkxError("Not all nodes have a 'time' attribute.")
    ...
    if n == 0:
        raise nx.py_networkxError("The cd index cannot be defined.")
