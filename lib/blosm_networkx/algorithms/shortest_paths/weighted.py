...
from lib.blosm_networkx.algorithms.shortest_paths.generic import _build_paths_from_predecessors
...
def dijkstra_path_length(G, source, target, weight="weight"):
    ...
    try:
        ...
        raise nx.py_networkxNoPath(f"Node {target} not reachable from {source}") from err
...
def multi_source_dijkstra(G, sources, target=None, cutoff=None, weight="weight"):
    ...
    try:
        ...
        raise nx.py_networkxNoPath(f"No path to {target}.") from err
...
def bellman_ford_predecessor_and_distance(
    ...
    Traceback (most recent call last):
        ...
    blosm_networkx.exception.py_networkxUnbounded: Negative cycle detected.
    ...
    In blosm_networkx v2.1 and prior, the source node had predecessor `[None]`.
    In blosm_networkx v2.2 this changed to the source node having predecessor `[]`
    """
    if source not in G:
        raise nx.NodeNotFound(f"Node {source} is not found in the graph")
    weight = _weight_function(G, weight)
    if G.is_multigraph():
        if any(
            ...
            raise nx.py_networkxUnbounded("Negative cycle detected.")
    else:
...
def single_source_bellman_ford(G, source, target=None, weight="weight"):
    ...
    try:
        ...
        raise nx.py_networkxNoPath(msg) from err
...
def goldberg_radzik(G, source, weight="weight"):
    ...
    py_networkxUnbounded
        ...
        As of blosm_networkx v3.2, a zero weight cycle is no longer
...
def negative_edge_cycle(G, weight="weight", heuristic=True):
    ...
    except nx.py_networkxUnbounded:
        return True
    finally:
...
def find_negative_cycle(G, source, weight="weight"):
    ...
    raise nx.py_networkxUnbounded(msg)
...
def bidirectional_dijkstra(G, source, target, weight="weight"):
    ...
    raise nx.py_networkxNoPath(f"No path between {source} and {target}.")
