...
from lib.blosm_networkx.utils import UnionFind, arbitrary_element, not_implemented_for
...
def all_pairs_lowest_common_ancestor(G, pairs=None):
    ...
    Parameters
    ----------
    G : blosm_networkx directed graph
    ...
    if not nx.is_directed_acyclic_graph(G):
        raise nx.py_networkxError("LCA only defined on directed acyclic graphs.")
    if len(G) == 0:
        raise nx.py_networkxPointlessConcept("LCA meaningless on null graphs.")
...
def tree_all_pairs_lowest_common_ancestor(G, root=None, pairs=None):
    ...
    Parameters
    ----------
    G : blosm_networkx directed graph (must be a tree)
    ...
    if root is None:
        for n, deg in G.in_degree:
            ...
            # checking deg>1 is not sufficient for MultiDiGraphs
                ...
                raise nx.py_networkxError(msg)
    if root is None:
        raise nx.py_networkxError("Graph contains a cycle.")
