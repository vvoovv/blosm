...
from lib.blosm_networkx.utils import not_implemented_for, py_random_state
...
def is_locally_k_edge_connected(G, s, t, k):
    ...
    Examples
    --------
    >>> from lib.blosm_networkx.algorithms.connectivity import is_locally_k_edge_connected
    >>> G = nx.barbell_graph(10, 0)
    ...
    try:
        if k <= 0:
            ...
            if not nx.is_k_edge_connected(G, k):
                raise nx.py_networkxUnfeasible("no available edges")
            aug_edges = []
        elif k == 1:
...
def bridge_augmentation(G, avail=None, weight=None):
    ...
    if G.number_of_nodes() < 3:
        raise nx.py_networkxUnfeasible("impossible to bridge connect less than 3 nodes")
    if avail is None:
...
def weighted_one_edge_augmentation(G, avail, weight=None, partial=False):
    ...
    if not partial and not nx.is_connected(meta_mst):
        raise nx.py_networkxUnfeasible("Not possible to connect G with available edges")
...
def weighted_bridge_augmentation(G, avail, weight=None):
    ...
    if len(avail) == 0:
        if nx.has_bridges(H):
            raise nx.py_networkxUnfeasible("no augmentation possible")
    ...
    #     contrary to blosm_networkx standards.  So, we actually need to run
    ...
    except nx.py_networkxException as err:
        # If there is no branching then augmentation is not possible
        raise nx.py_networkxUnfeasible("no 2-edge-augmentation possible") from err
...
def greedy_k_edge_augmentation(G, k, avail=None, weight=None, seed=None):
    ...
    if not done:
        raise nx.py_networkxUnfeasible("not able to k-edge-connect with available edges")
