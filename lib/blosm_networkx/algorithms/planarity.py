...
def check_planarity(G, counterexample=False):
    ...
    Returns
    -------
    (is_planar, certificate) : (bool, blosm_networkx graph) tuple
...
def get_counterexample(G):
    ...
    Raises nx.py_networkxException if G is planar.
    ...
    if check_planarity(G)[0]:
        raise nx.py_networkxException("G is planar - no counter example.")
    ...
    def traverse_face(self, v, w, mark_half_edges=None):
        ...
        while cur_node != v or prev_node != incoming_node:
            ...
            if (prev_node, cur_node) in mark_half_edges:
                raise nx.py_networkxException("Bad planar embedding. Impossible face.")
