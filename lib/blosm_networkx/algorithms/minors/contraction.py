...
from lib.blosm_networkx import density
from lib.blosm_networkx.exception import NetworkXException
...
def quotient_graph(
    ...
    *`Strongly connected component`_*.
    ...
    if not nx.community.is_partition(G, partition):
        raise py_networkxException("each node must be in exactly one part of `partition`")
    return _quotient_graph(
        G,
        partition,
        edge_relation,
