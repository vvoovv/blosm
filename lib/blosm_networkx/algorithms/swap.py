...
def directed_edge_swap(G, *, nswap=1, max_tries=100, seed=None):
    ...
    """
    if nswap > max_tries:
        raise nx.py_networkxError("Number of swaps > number of tries allowed.")
    if len(G) < 4:
        raise nx.py_networkxError("DiGraph has fewer than four nodes.")
    if len(G.edges) < 3:
        raise nx.py_networkxError("DiGraph has fewer than 3 edges")
    ...
    while swapcount < nswap:
        ...
        if tries > max_tries:
            ...
            raise nx.py_networkxAlgorithmError(msg)
...
def double_edge_swap(G, nswap=1, max_tries=100, seed=None):
    ...
    if nswap > max_tries:
        raise nx.py_networkxError("Number of swaps > number of tries allowed.")
    if len(G) < 4:
        raise nx.py_networkxError("Graph has fewer than four nodes.")
    if len(G.edges) < 2:
        raise nx.py_networkxError("Graph has fewer than 2 edges")
    # Instead of choosing uniformly at random from a generated edge list,
    # this algorithm chooses nonuniformly from the set of nodes with
    # probability weighted by degree.
    n = 0
    ...
    while swapcount < nswap:
        ...
        if n >= max_tries:
            ...
            raise nx.py_networkxAlgorithmError(e)
        n += 1
    return G
...
def connected_double_edge_swap(G, nswap=1, _window_threshold=3, seed=None):
    ...
    if not nx.is_connected(G):
        raise nx.py_networkxError("Graph not connected")
    if len(G) < 4:
        raise nx.py_networkxError("Graph has fewer than four nodes.")
    n = 0
    swapcount = 0
