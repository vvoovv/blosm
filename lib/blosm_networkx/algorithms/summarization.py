...
def dedensify(G, threshold, prefix=None, copy=True):
    ...
    Returns
    -------
    dedensified blosm_networkx graph : (graph, set)
    ...
    """
    if threshold < 2:
        raise nx.py_networkxError("The degree threshold must be >= 2")
...
def _snap_build_graph(
    ...
    Returns
    -------
    summary graph: blosm_networkx graph
    """
    output = G.__class__()
    node_label_lookup = {}
...
def snap_aggregation(
    ...
    ----------
    G: graph
        blosm_networkx Graph to be summarized
    ...
    Returns
    -------
    blosm_networkx.Graph: summary graph
