...
def eulerian_circuit(G, source=None, keys=False):
    ...
    if not is_eulerian(G):
        raise nx.py_networkxError("G is not Eulerian.")
    if G.is_directed():
        G = G.reverse()
    else:
        G = G.copy()
...
def eulerian_path(G, source=None, keys=False):
    ...
    if not has_eulerian_path(G, source):
        raise nx.py_networkxError("Graph has no Eulerian paths.")
    if G.is_directed():
        G = G.reverse()
...
def eulerize(G):
    ...
    """
    if G.order() == 0:
        raise nx.py_networkxPointlessConcept("Cannot Eulerize null graph")
    if not nx.is_connected(G):
        raise nx.py_networkxError("G is not connected")
