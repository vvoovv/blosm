...
from lib.blosm_networkx.utils.decorators import py_random_state
...
def diameter(G, seed=None):
    ...
    See Also
    --------
    blosm_networkx.algorithms.distance_measures.diameter
    ...
    # if G is empty
    if not G:
        raise nx.NetworkXError("Expected non-empty blosm_networkx graph!")
...
def _two_sweep_undirected(G, seed):
    """Helper function for finding a lower bound on the diameter
        ...
        ``G`` is a blosm_networkx undirected graph.
...
def _two_sweep_directed(G, seed):
    """Helper function for finding a lower bound on the diameter
        ...
        ``G`` is a blosm_networkx directed graph.
