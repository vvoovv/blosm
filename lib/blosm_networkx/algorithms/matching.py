...
def matching_dict_to_set(matching):
    ...
    In the definition of maximal matching adopted by blosm_networkx,
    ...
    for edge in matching.items():
        ...
        if (v, u) in edges or edge in edges:
            continue
        if u == v:
            raise nx.py_networkxError(f"Selfloops cannot appear in matchings {edge}")
        edges.add(edge)
    return edges
...
def is_matching(G, matching):
    ...
    for edge in matching:
        if len(edge) != 2:
            raise nx.py_networkxError(f"matching has non-2-tuple edge {edge}")
        u, v = edge
        if u not in G or v not in G:
            raise nx.py_networkxError(f"matching contains edge {edge} with node not in G")
        if u == v:
            return False
...
def max_weight_matching(G, maxcardinality=False, weight="weight"):
    ...
    :mod:`blosm_networkx.algorithms.bipartite.matching`.
