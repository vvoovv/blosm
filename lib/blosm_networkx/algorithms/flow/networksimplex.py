...
from lib.blosm_networkx.utils import not_implemented_for
...
def network_simplex(G, demand="demand", capacity="capacity", weight="weight"):
    ...
    if sum(DEAF.node_demands) != 0:
        raise nx.py_networkxUnfeasible("total node demand is not zero")
    for e, c in zip(DEAF.edge_indices, DEAF.edge_capacities):
        if c < 0:
            raise nx.py_networkxUnfeasible(f"edge {e!r} has negative capacity")
    if not multigraph:
        edges = nx.selfloop_edges(G, data=True)
    else:
        edges = nx.selfloop_edges(G, data=True, keys=True)
    for e in edges:
        if e[-1].get(capacity, inf) < 0:
            raise nx.py_networkxUnfeasible(f"edge {e[:-1]!r} has negative capacity")
    ...
    if any(DEAF.edge_flow[i] != 0 for i in range(-n, 0)):
        raise nx.py_networkxUnfeasible("no flow satisfies all node demands")
    ...
    if any(DEAF.edge_flow[i] * 2 >= faux_inf for i in range(DEAF.edge_count)) or any(
        ...
        raise nx.py_networkxUnbounded("negative cycle with infinite capacity found")
