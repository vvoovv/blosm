...
from lib.blosm_networkx.utils import arbitrary_element, not_implemented_for, pairwise
...
def has_cycle(G):
    ...
    except nx.py_networkxUnfeasible:
        return True
    else:
        return False
...
def topological_generations(G):
    ...
    py_networkxError
        ...
        `G` is undirected, a :exc:`py_networkxError` is raised.

    py_networkxUnfeasible
        If `G` is not a directed acyclic graph (DAG) no topological generations
        exist and a :exc:`py_networkxUnfeasible` exception is raised.  This can also
    ...
    if not G.is_directed():
        raise nx.py_networkxError("Topological sort not defined on undirected graphs.")
    ...
    if indegree_map:
        raise nx.py_networkxUnfeasible(
...
def topological_sort(G):
    ...
    py_networkxError
        ...
        is undirected, a :exc:`py_networkxError` is raised.

    py_networkxUnfeasible
        If `G` is not a directed acyclic graph (DAG) no topological sort exists
        and a :exc:`py_networkxUnfeasible` exception is raised.  This can also be
    ...
    with :func:`blosm_networkx.line_graph` as follows:
...
def lexicographical_topological_sort(G, key=None):
    ...
    if indegree_map:
        ...
        raise nx.py_networkxUnfeasible(msg)
...
def all_topological_sorts(G):
    ...
    while True:
        ...
        if len(current_sort) == len(G):
            ...
            if len(D) == 0:
                raise nx.py_networkxUnfeasible("Graph contains a cycle.")
...
def is_aperiodic(G):
    ...
    """
    if not G.is_directed():
        raise nx.py_networkxError("is_aperiodic not defined for undirected graphs")
...
def transitive_closure(G, reflexive=False):
    ...
    if reflexive not in {None, True, False}:
        raise nx.py_networkxError("Incorrect value for the parameter `reflexive`")
...
def transitive_reduction(G):
    ...
    py_networkxError
        ...
        not uniquely defined and a :exc:`py_networkxError` exception is raised.
...
def antichains(G, topo_order=None):
    ...
    for the SAGE project. It's included in blosm_networkx with permission from the
...
def dag_to_branching(G):
    ...
    As described in :mod:`blosm_networkx.algorithms.tree.recognition`, a
    ...
    *arborescences*. For this function, each node of in-degree zero in
    ...
    :func:`blosm_networkx.convert_node_labels_to_integers` function.

    The current implementation of this function uses
    :func:`blosm_networkx.prefix_tree`, so it is subject to the limitations of
    that function.
