...
def min_edge_cover(G, matching_algorithm=None):
    ...
    matching_algorithm : function
        ...
        :func:`~blosm_networkx.algorithms.matching.max_weight_matching` is used.
        Common bipartite matching functions include
        :func:`~blosm_networkx.algorithms.bipartite.matching.hopcroft_karp_matching`
        or
        :func:`~blosm_networkx.algorithms.bipartite.matching.eppstein_matching`.
    ...
    function in :mod:`blosm_networkx.algorithms.bipartite.covering` which is
    simply this function with a default matching algorithm of
    :func:`~blosm_networkx.algorithms.bipartite.matching.hopcraft_karp_matching`
    """
    if len(G) == 0:
        return set()
    if nx.number_of_isolates(G) > 0:
        # ``min_cover`` does not exist as there is an isolated node
        raise nx.py_networkxException(
