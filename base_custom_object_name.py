...
# custom setup begins
#
# redefine Renderer.getName
#
from renderer import Renderer
def _getName(self, element):
    tags = element.tags
    return "%s,%s" % (tags["addr:street"], tags["addr:housenumber"])\
        if "addr:street" in tags and "addr:housenumber" in tags else\
        ( tags.get("name") if "name" in tags else "element" )
Renderer.getName = _getName
# custom setup ends
